from clean_csv import convert, prefixes
import pandas as pd
import re
...
def update_components_col(df):
    ...
    for row in df.itertuples():
        gloss = row.og_gloss
        df.loc[df["og_gloss"] == gloss, "components"] = "+".join(clean_glosses(gloss))
...
update_components_col(df)
update_display_components_col(df)
update_handedness_col(df)
df = df.drop(columns=['variant_1', "variant_2"])
